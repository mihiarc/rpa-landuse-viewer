#!/usr/bin/env python3
"""
Add ensemble scenarios for GCMs to the RPA Land Use database.

This script:
1. Identifies all unique RCP+SSP combinations
2. For each combination, creates an ensemble scenario that averages across all GCMs
...
logger = logging.getLogger("gcm-ensemble-creator")

def get_unique_rcp_ssp_combinations():
    """Get all unique combinations of RCP and SSP in the database."""
    query = """
    SELECT DISTINCT rcp, ssp
    FROM scenarios
    WHERE rcp != 'ensemble' AND ssp != 'ensemble'  -- Exclude any existing ensemble scenarios
    ORDER BY rcp, ssp
    """
...
def get_scenarios_for_rcp_ssp(rcp, ssp):
    """Get all scenarios for a given RCP and SSP combination."""
    query = """
    SELECT scenario_id, scenario_name, gcm
    FROM scenarios
    WHERE rcp = ? AND ssp = ?
    """
    return DBManager.query_df(query, [rcp, ssp])

def create_ensemble_scenario(ensemble_name, rcp, ssp):
    """Create a new scenario record for a GCM ensemble scenario."""
    ...
    description = f"Mean transition across all GCMs for {rcp} and {ssp}"
    
    with DBManager.connection() as conn:
        conn.execute(insert_query, [ensemble_id, ensemble_name, rcp, ssp, description])
...
def handle_ensemble_for_rcp_ssp(rcp, ssp, force_recreate=False):
    """Handle the creation of an ensemble scenario for a specific RCP and SSP combination."""
    ensemble_name = f"ensemble_gcm_{rcp}_{ssp}"
    ...
    # Get scenarios for this RCP+SSP combination
    scenarios_df = get_scenarios_for_rcp_ssp(rcp, ssp)
    scenario_ids = scenarios_df['scenario_id'].tolist()
    
    if not scenario_ids:
        logger.warning(f"No scenarios found for RCP={rcp}, SSP={ssp}")
        return
    
    logger.info(f"Found {len(scenario_ids)} scenarios for RCP={rcp}, SSP={ssp}: {', '.join(scenarios_df['gcm'].tolist())}")
    
    # Create the new ensemble scenario
    ensemble_id = create_ensemble_scenario(ensemble_name, rcp, ssp)
    
    # Calculate and insert the ensemble transitions
    calculate_and_insert_ensemble_transitions(ensemble_id, scenario_ids)
    
    logger.info(f"GCM ensemble scenario '{ensemble_name}' created successfully with ID: {ensemble_id}")
    return ensemble_id

def main():
    """Main function to create GCM ensemble scenarios."""
    logger.info("Starting creation of GCM ensemble scenarios")
    
    # Get all unique RCP+SSP combinations
    combinations = get_unique_rcp_ssp_combinations()
    logger.info(f"Found {len(combinations)} unique RCP+SSP combinations")
    
    # For each combination, create an ensemble
    for _, row in combinations.iterrows():
        rcp = row['rcp']
        ssp = row['ssp']
        logger.info(f"Processing RCP={rcp}, SSP={ssp}")
        handle_ensemble_for_rcp_ssp(rcp, ssp)
    
    # Optimize the database after all inserts
    logger.info("Optimizing database")
    SchemaManager.optimize_database()
    
    logger.info("All GCM ensemble scenarios created successfully")
